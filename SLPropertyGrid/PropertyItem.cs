...
namespace SLPropertyGrid
{
	...
	public sealed class PropertyItem : INotifyPropertyChanged
	{
		...
		public PropertyItem(object instance, object value, PropertyInfo property, bool readOnly, Action somethingChanged)
		{
			_instance = instance;
		...
		public object Value
		{
			...
			set
			{
				...
				try
				{
					var propertyType = this._propertyInfo.PropertyType;
					...
					else
					{
						try
						{
							...
							{
								var tc = TypeConverterHelper.GetConverter(propertyType);
								if (tc != null)
								{
									var convertedValue = tc.ConvertFrom(value);
						...
						{
							// don't reset the value here so that a number field can be temporarily in a bad state (ie typing .0)
		...
		private void OnPropertyChanged(string propertyName)
		{
            ...
            if (propertyName == "Value" && SomethingChanged != null)
                SomethingChanged();
		}
