...
namespace Graphs
{
    public class Graph : GraphicsLayer.IPaintable
    {
        ...
        {
            var serializer = new DataContractJsonSerializer(typeof(SerializationGraph));

            using (var m = new MemoryStream())
            {
                serializer.WriteObject(m, new SerializationGraph(this));
                m.Position = 0;
        ...
        public static Graph Deserialize(string s)
        {
            ...
            using (var sw = new StreamWriter(m))
            {
                ...
                try
                {
                    var serializedGraph = serializer.ReadObject(m) as SerializationGraph;
                    return new Graph(serializedGraph);
                }
                catch { }
            }
